...
- `findings/security.md`、`findings/reliability.md`、`findings/performance.md`、`findings/cost.md`（格式見 `.claude/skills/report-aws/templates/finding-format.md`）
...
唯一資料來源，之後由 `.claude/skills/report-aws/scripts/build-report.js` 確定性產生 HTML，不經過 LLM。
...
- 欄位定義、**必填與驗證規則**（`build-report.js` 會強制檢查、不合即 exit 1）都完整列在
  ...
  **spec 即完整契約，不要去讀 `.claude/skills/report-aws/scripts/build-report.js` 原始碼**（讀它只是把 14K 字元的
