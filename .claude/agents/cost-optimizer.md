...
- 需要補查時只能用唯讀 AWS CLI（describe/list/get，含 `ce` 與 `cloudwatch get-metric-statistics`）。**帶時間窗一律填 `data/scan-meta.json` 的字面時間戳**（`ce` 用 `cost_window` 的 `start`／`end`，CloudWatch 用 `metrics_window`）：先用 Read 讀出後直接填入指令；**嚴禁在 aws 指令內用 `$(date …)` 命令替換**——會觸發權限確認、破壞無人值守
