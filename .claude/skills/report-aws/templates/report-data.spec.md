...
```bash
node .claude/skills/report-aws/scripts/build-report.js                # 讀 report/report-data.json → report/aws-report.html
node .claude/skills/report-aws/scripts/build-report.js --standalone   # 包成完整 HTML 供本機直接開啟
node .claude/skills/report-aws/scripts/build-report.js --theme .claude/skills/report-aws/templates/themes/<專案>.css   # 換專案主題
node .claude/skills/report-aws/scripts/build-report.js --masked       # 對外分享版：啟用遮罩防呆檢查
```

版型、配色、章節結構凍結在 `.claude/skills/report-aws/templates/report.html.template` 與
`.claude/skills/report-aws/templates/themes/*.css`，逐月只換本檔的資料。完整範例見
`.claude/skills/report-aws/templates/report-data.example.json`（可直接產出一份與設計基準一致的報告）。

## 遮罩原則

報告為正式上線用途，**預設不遮罩**：帳號 ID、資源 ID、網域名稱照實填寫。
僅在使用者明確要求對外分享版時才另外產生遮罩版資料，並以 `--masked` 建置——
此模式會對最終 HTML 做防呆掃描（12 位數字、`AKIA`/`ASIA` 金鑰樣式、ARN、內部私有 IP、Email），命中即中止不寫檔。
...
以下是產生器實際執行的全部驗證。**本節即完整契約，不需要去讀 `.claude/skills/report-aws/scripts/build-report.js` 原始碼。**
...
**pillars**
- 必須**恰好 4 個**，且 `id` 依序為 `security` → `reliability` → `performance` → `cost`
- `score`：1–5 的整數
- `high` / `medium` / `low`：≥0 的整數
- `findings`：必須是陣列（可為空陣列）
- `findings[].id`：須符合 `<支柱前綴>-<流水號>`（`SEC-` / `REL-` / `PERF-` / `COST-`），且前綴要對得上所屬支柱
...
- 必須是 **1–5 項**
- `title`、`desc`：必填、非空
- `tags`：必須是陣列。其中**長得像發現編號的字串**（`SEC-01` 等）會被驗證**必須存在於某支柱的 findings 明細中**——
...
**method**
- 必須是物件（`items`、`gaps` 見上方欄位定義）

## 換專案主題

複製 `.claude/skills/report-aws/templates/themes/default.css` 為 `<專案>.css` 改色票即可；
