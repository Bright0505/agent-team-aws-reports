...
performance-reviewer 的引用來源（依支柱拆分，只讀這一份即可；使用規則與背景見 `references/aws-docs-common.md`）。最後驗證：2026-07-24。
...
- [AWS Fargate Linux platform version deprecation](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/platform-versions-retired.html) — 平台版本淘汰時間表（1.3.0 已於 2026-06-15 進入 deprecation date）
...
- [Supported node types (ElastiCache)](https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/CacheNodes.SupportedTypes.html) — ElastiCache 節點型別世代
- [Amazon Redshift clusters](https://docs.aws.amazon.com/redshift/latest/mgmt/working-with-clusters.html) — Redshift 節點型別（RA3）與叢集規格
- [Enabling API caching to enhance responsiveness](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-caching.html) — API Gateway 快取
