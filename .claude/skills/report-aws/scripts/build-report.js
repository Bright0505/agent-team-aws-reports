#!/usr/bin/env node
/**
 * 確定性報告產生器：report-data.json + 模板 + 主題 → report/aws-report.html
...
const PILLAR_DEFS = [
  ...
  { id: 'performance', name: '效能效率', en: 'Performance', code: 'PERF' },
...
function prepare(data) {
  ...
  meta.eyebrow = meta.eyebrow || 'AWS Well-Architected · 基礎架構評估';
  meta.regions_display = meta.regions.join('、');
  meta.lede_html = inline(meta.lede, 'strong');

  // pillars：固定四支柱、固定順序
  need(Array.isArray(data.pillars) && data.pillars.length === 4, 'pillars 必須恰好 4 個（security/reliability/performance/cost 順序固定）');
  if (errs.length) return errs;
  ...
  (data.priorities || []).forEach((it, i) => {
    ...
    (it.tags || []).forEach((t) => {
      const m = String(t).match(/^(SEC|REL|PERF|COST)-\d+$/);
