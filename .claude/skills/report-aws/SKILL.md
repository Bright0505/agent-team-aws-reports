...
- **只有成本**跟著期別：成本窗＝主體週期＋前 N 期趨勢（月報＋前 2 月、季報＋前 1 季、年報＋前 1 年，
  皆 MONTHLY；季報／年報的前 N 為暫定值，定義在 `.claude/skills/report-aws/scripts/scan.sh` 的 `SPAN`/`TREND_PRIOR`）。
...
派 `aws-scanner`（同步執行、等它完成）跑 `bash .claude/skills/report-aws/scripts/scan.sh default <期別>` 掃描帳號
（期別當第二個位置參數傳入以決定成本趨勢窗；留空則 `bash .claude/skills/report-aws/scripts/scan.sh`，預設上一個完整月）。
profile `default` 應設定為**唯讀憑證**（IAM 掛 `ReadOnlyAccess`）——唯讀鐵則的強制層在 IAM；
若唯讀金鑰放在其他 profile，第一個參數改用該名稱。
相對路徑形式跨機器與改專案名都成立。
掃什麼：核心掃描與明細迴圈寫死在 scan.sh；「一次性 list/describe」服務由 `scan-catalog.json`（版控基線）
＋專案根目錄 `scan-catalog.local.json`（gitignored 補充）宣告，scan.sh 自動讀取——要為某帳號加必掃服務時改 catalog 即可，不動腳本。
...
另會產出 **`data/digest/coverage.md`**（服務覆蓋率自檢：帳號實際用到的服務 vs scan.sh 掃到的，
標出「有計費卻沒掃到」的缺口）——供成本分析點名未盤點服務，也讓覆蓋率缺口每次掃描自動浮現。
...
```
bash .claude/skills/report-aws/scripts/check-links.sh report/AWS架構報告.md findings/security.md findings/reliability.md findings/performance.md findings/cost.md
```

- 全數有效 → 進入階段 ⑥。
- 有失效連結（exit 1）→ **不要中止流程**。把失效清單記下來，在收尾摘要中列出，
  並提醒需更新 `.claude/skills/report-aws/references/` 下對應支柱的目錄檔。
  （`docs.aws.amazon.com` 失效頁面仍回 HTTP 200，只能靠這支腳本判斷，WebFetch 看不出來。）

## 階段 ⑥ — 存檔本期報告（由你＝主對話執行）

```
bash .claude/skills/report-aws/scripts/archive-report.sh
```
...
- 連結檢查結果（全數有效／或列出失效連結與待更新的 `.claude/skills/report-aws/references/aws-docs-<支柱>.md`）
- 產出檔路徑：`report/aws-report.html`、`report/AWS架構報告.md`、`report/report-data.json`
- 提醒：如需對外分享版，可再要求 `node .claude/skills/report-aws/scripts/build-report.js --masked`（遮罩防呆）或發佈 Artifact
