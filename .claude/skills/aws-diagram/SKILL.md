---
name: aws-diagram
description: 把 /report-aws 的掃描產物（data/）確定性轉成 draw.io 架構圖（全景架構頁＋架構索引頁＋每 VPC 一頁，子網級拓撲）。選配功能，想到才跑，不在無人值守流程內。
---
...
帳號長大到多區域多 VPC 時，頁 1 會失去一張看完的價值，頁 2 才是索引。

- 頁 1「全景架構」：全帳號一張，給對外簡報用。雲外入口欄（使用者 → Route 53 → CloudFront）
...
- 頁 2「架構索引」：使用者 → CloudFront（標 alias；停用者灰化）→ 各 VPC 縮略框／S3。
  無工作負載的 VPC（如 default VPC）在此只有縮略框（但全景架構頁仍會完整畫出其子網）。
